...
/*   Updated: 2025/03/27 12:21:44 by iunikel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     calculate_think_time(t_philo *philo)
{
        ...
        // Use a minimal fixed value to prevent CPU spinning
        think_time = 1;
        
        // If there's remaining time before potential death, use it
        if (philo->data->time_to_die > philo->data->time_to_eat 
                + philo->data->time_to_sleep)
        {
                // Use the full remaining time
                think_time = philo->data->time_to_die - philo->data->time_to_eat 
                        - philo->data->time_to_sleep - 10; // Small safety margin
        }
        
        return (think_time > 0 ? think_time : 1);
}
